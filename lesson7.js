...
var bombTimer; //Таймер для бомбочки
...
function startGame() {
   ...
   // setTimeout(createBomb, 6000);
    bombTimer = setInterval(createBomb, 6000);
...
function move() {
   ...
   //********Если змейка врезалась в край поля, переопределяем ее координаты****//// 
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        gameSpan.innerText='Счет:'+score;
        gameScore.append(gameSpan);
...
function createFood() {
    ...
    while (!foodCreated) { //пока еду не создали
        ...
        // проверка на змейку и на бомбочку
